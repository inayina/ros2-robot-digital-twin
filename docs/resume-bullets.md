...
- 设计并实现 STM32F411 + MPU6050 嵌入式监测与控制固件，基于 FreeRTOS 搭建 100 Hz 传感器采样、6 轴姿态解算、窗口 RMS 状态判别、LED 报警和电机执行任务链路。
- 将 6 轴姿态解算下沉到 STM32，输出 `IMUQ` 四元数串口帧，使 ROS 2 `sensor_msgs/Imu.orientation` 可使用真实姿态数据。
- 开发 ESP32-S3 micro-ROS 无线桥接模块，模块化重构 UART 解析与 ROS 2 pub/sub 逻辑，以 `best_effort` QoS 发布 `/imu/data`、`/imu/filtered`、`/robot/state`，并打通 `/cmd_vel -> CMDVEL -> STM32 -> TB6612` 下行控制链路。
...
- Built STM32F411 + MPU6050 embedded monitoring-and-control firmware with FreeRTOS tasks for 100 Hz sensor sampling, 6-axis attitude estimation, RMS-based state classification, LED alarm output, and motor actuation.
...
- Implemented an ESP32-S3 micro-ROS bridge with modular UART parsing and ROS 2 pub/sub logic, using `best_effort` QoS for `/imu/data`, `/imu/filtered`, and `/robot/state`, while bridging `/cmd_vel -> CMDVEL -> STM32 -> TB6612` for downstream motor control.
